...
### patch
```cpp
  return (base::Time::Now() - base::Days(base::FeatureList::IsEnabled(history::kHistoryMoreSearchResults)  \
           ? kBraveLowQualityMatchAgeLimitInDays                         \
           : kLowQualityMatchAgeLimitInDays));                                                      \
  /* DCHECK, to make sure we aren't overriding something we shouldn't */ \
  DCHECK(kLowQualityMatchAgeLimitInDays == kLowQualityMatchAgeLimitInDays);
