...
>>> 
 int 
 GetDataVersion(PrefService* prefs) 
 { 
<<< 
...
### patch
```cpp
int GetDataVersion_ChromiumImpl(PrefService* prefs) {
...
### patch
```cpp
std::unique_ptr<TemplateURLData> GetPrepopulatedEngine_Unused(
...
### patch
```cpp
std::unique_ptr<TemplateURLData> GetPrepopulatedFallbackSearch_Unused(
